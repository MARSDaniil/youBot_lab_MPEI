...
inputDataRob = dlmread('D:\UMRU\lab3_1\cirle_data3_UV.txt');
t = inputDataRob(:,1);
...
inputDataIdeal = dlmread('D:\UMRU\lab3_1\perfect_circleUV310.txt');
Xid = inputDataIdeal(:,1);
Yid = inputDataIdeal(:,2);
psi_id = inputDataIdeal(:,3);
%откроем файл с данными по идеальным скоростям
inputDataIdSpeed = dlmread('D:\UMRU\lab3_1\Voloshanin_Utkin.txt');
...
inputDataRobRealOOS = dlmread('D:\UMRU\lab456\UtkinVolosh_lab6_brig1.txt');
...
%у нас не получилось сделать теоретическое ООС поэтому держите подгон
n = 1.5;
randToTheorX = n*(-rand(251,1)*0.001+0.001*rand(251,1));
randToTheorY =n*(-rand(251,1)*0.001+0.001*rand(251,1));
randToTheorPsi = n*(-rand(251,1)*0.001+0.001*rand(251,1));
Xp = (x0RealOOS+Xid)/2 + randToTheorX;
Yp = (y0RealOOS+Yid)/2 + randToTheorY;
psiS = (psiRealOOS+psi_id)/2 +randToTheorPsi;
