...
inputDataRob = dlmread('D:\UMRU\lab3\cirle_data3_UV.txt');
t = inputDataRob(:,1);
...
inputDataIdeal = dlmread('D:\UMRU\lab3\perfect_circleUV3.txt');
Xid = inputDataIdeal(:,1);
Yid = inputDataIdeal(:,2);
psi_id = inputDataIdeal(:,3);
%откроем файл с данными по идеальным скоростям
inputDataIdSpeed = dlmread('D:\UMRU\lab3\Voloshanin_Utkin.txt');
...
%dXsr dYsr delPsisr так же будут матОжиданием
dXsr_iv = dXsr_iv/length(dVx); dYsr_iv = dYsr_iv/length(dVx); delPsisr_iv = delPsisr_iv/length(dVx); 
